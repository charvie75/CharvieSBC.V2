...
LDNXT:
	jsr RXCHAR				; get character from UART
	cmp #' '
...
LDEOF:
	...
	jsr NEWLINE
LD056:
	jsr GETCHAR				; wait for 'CR'
	cmp #$0d				; Is it a 'CR'
	bne LD056				; If 'NO' go back and check again 
	jmp WOZMON
...
;---------------------------------------------------       
LDBRK:
	lda #'B'				; received BREAK (ESC)
	.byte   $2C
LDERI:
	lda #'R'				; unknown record identifier error
	.byte   $2C
LDECS:
	lda #'C'				; checksum error
	.byte   $2C
LDEIC:
	lda #'I'				; input character error
	.byte   $2C
LDEM:
	lda #'M'				; memory error
	jsr TXCHAR				; print it
...
LDNIB:
	jsr GETCHAR				; get character from UART
